...
function Tip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 6, padding: '8px 12px', fontSize: 11, boxShadow: `0 4px 12px ${t.shadow}` }}>
      {label && <p style={{ color: t.textMuted, marginBottom: 4 }}>{label}</p>}
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color, margin: '2px 0' }}>{p.name}: <strong style={{ color: t.textPrimary }}>{p.value}</strong></p>
      ))}
    </div>
  )
}
...
export default function TrendChart({ type = 'line', title, data, lines, height = 210 }) {
  const { t } = useTheme()
  const axisStyle = { stroke: t.textSubtle, tick: { fontSize: 10, fill: t.textMuted } }
  ...
  return (
    <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8, padding: '14px 16px' }}>
      {title && <h3 style={{ margin: '0 0 10px', fontSize: 12, fontWeight: 600, color: t.textPrimary }}>{title}</h3>}
