...
export default function ActionList({ title, items }) {
  const { t } = useTheme()
  return (
    <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
        <h3 style={{ margin: 0, fontSize: 13, fontWeight: 600, color: t.textPrimary }}>{title}</h3>
        <span style={{ fontSize: 11, color: t.textSubtle }}>{items.length} items</span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, color: t.textPrimary, lineHeight: 1.4, marginBottom: 3 }}>{item.text}</div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {item.category && <span style={{ fontSize: 10, color: t.textMuted, background: t.bgTag, padding: '1px 6px', borderRadius: 4 }}>{item.category}</span>}
                  {item.due && (
              ...
              <ArrowRight size={11} color={t.textSubtle} style={{ flexShrink: 0, marginTop: 2 }} />
            </div>
          )
        })}
      </div>
