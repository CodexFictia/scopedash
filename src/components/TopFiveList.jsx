...
export default function TopFiveList({ title, items, valueLabel }) {
  ...
  return (
    <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8, padding: '14px 16px' }}>
      <h3 style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 600, color: t.textPrimary, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  ...
                  fontSize: 10, fontWeight: 700, color: i < 3 ? RANK_COLORS[i] : t.textSubtle,
                  background: i < 3 ? RANK_COLORS[i] + '20' : t.bgTag, flexShrink: 0,
                }}>{i + 1}</span>
                <span style={{ fontSize: 12, color: t.textPrimary }}>{it.label}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                {it.change != null && (it.change > 0 ? <TrendingUp size={10} color="#3fb950" /> : <TrendingDown size={10} color="#f85149" />)}
                <span style={{ fontSize: 12, fontWeight: 600, color: t.textPrimary }}>{it.value}</span>
                {valueLabel && <span style={{ fontSize: 10, color: t.textMuted }}>{valueLabel}</span>}
              </div>
            </div>
